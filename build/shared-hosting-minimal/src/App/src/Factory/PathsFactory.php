...
class PathsFactory
{
    public function __invoke(ContainerInterface $container): Paths
    {
        ...
        $paths = Paths::withPreset($preset, $basePath);

        // Apply custom paths if provided
        if (! empty($customPaths)) {
            $paths = new Paths($basePath, array_merge($paths->all(), $customPaths));
        }

        // Configure security if enabled
        if (isset($pathsConfig['security']) && is_array($pathsConfig['security'])) {
            /** @var array<string, mixed> $securityConfigArray */
            $securityConfigArray = $pathsConfig['security'];
            $securityConfig      = $this->createSecurityConfig($securityConfigArray);
            $paths->setSecurityConfig($securityConfig);
        }
